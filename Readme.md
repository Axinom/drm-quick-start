...
* Your website or client app must use HTTPS in order to protect against the interception of communications (e.g. to steal the license token generated by the authorization service). The Axinom DRM license server does not allow you to communicate with it in an insecure way but communication channels inside your solution must be secured by you.
